...
export default function LeadScraper({ ses, onImported }: LeadScraperProps) {
  ...
  const handleImport = async () => {
    if (!targetSeId) {
      toast.error('Selecteer een Sales Executive');
      return;
    }
    if (selected.size === 0) {
    ...
    if (error) {
      ...
      toast.success(`${leadsToImport.length} lead(s) geïmporteerd`);
      setResults([]);
      setSelected(new Set());
      onImported();
    }
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-lg">Lead Scraper</CardTitle>
          <p className="text-sm text-muted-foreground">
            Zoek op branche + regio om bedrijven te vinden met contactgegevens
          </p>
        </CardHeader>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Select value={targetSeId} onValueChange={setTargetSeId}>
                  <SelectTrigger className="w-[200px]">
                    <SelectValue placeholder="Toewijzen aan SE..." />
                  </SelectTrigger>
                  <SelectContent>
                    {ses.map(se => (
                ...
                <Button
                  ...
                  disabled={importing || selected.size === 0 || !targetSeId}
                  size="sm"
                >
                  {importing ? (
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1.5">
                            ...
                            {dup && (
                              <Badge variant="outline" className="text-xs ml-1">Bestaat al</Badge>
                            )}
                          </div>
                        </TableCell>
                        <TableCell>
